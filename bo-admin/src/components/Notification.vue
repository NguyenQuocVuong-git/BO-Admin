...
<script setup>
import { ref } from 'vue';
import  useDropdown  from '@/utils/customHooks';
...
<template>
  <li class="relative" :class="{ 'dark': darkMode }" @click.prevent="toggleDropdown" ref="dropdownRef" >
    <a class="relative flex h-8.5 w-8.5 items-center justify-center rounded-full border-[0.5px] border-stroke bg-gray hover:text-primary dark:border-strokedark dark:bg-meta-4 dark:text-white"
      href="#">
      <span v-if="notifying" class="absolute -top-0.5 right-0 z-1 h-2 w-2 rounded-full bg-meta-1">
      ...
      <ul class="flex h-auto flex-col overflow-y-auto">
        <li v-for="(notification, index) in notifications" :key="index">
          <a :href="notification.link"
            ...
            <p class="text-sm">
              <span :class="darkMode ? 'text-white' : 'text-black'">{{ notification.title }}</span>
